...
int estateSearch(struct gameState *state, int currentPlayer){
	int p = 0;
...
//Function for the baron case
int baronEffect(int choice1, struct gameState *state, int currentPlayer) {
        state->numBuys++;//Increase buys by 1!
        if (choice1 > 0) { //Boolean true or going to discard an estate
            int p = estateSearch(state, currentPlayer); //search for estate card
	        if (p != -1){
...
int minionEffect(int handPos, struct gameState *state, int currentPlayer, int choice2, int choice1){
	...
	int i;
        ...
        else if (choice2)	//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
        {
            ...
            discardDraw(currentPlayer, state, handPos);
...
//helper function that finds a particular card in a player's hand and discards it
void findDiscard(int currentPlayer, struct gameState *state, int j) {
...
int mineEffect(struct gameState *state, int choice1, int choice2, int currentPlayer, int handPos){
        ...
        findDiscard(currentPlayer, state, j);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case minion:
	return minionEffect(handPos, state, currentPlayer, choice2, choice1);

    case ambassador:
	return ambassadorEffect(state, choice1, choice2, handPos, currentPlayer);
