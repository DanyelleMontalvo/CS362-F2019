    ...
    {                               //Boolean true or going to discard an estate
        ...
        while (card_not_discarded)
        {
            ...
            {                      //Found an estate card!
                ...
                if (supplyCount(estate, state) > 1) { // introduced bug: supply count won't dip below 1
...
int mineEffect(int currentPlayer, int card, int choice1, int choice2, struct gameState *state, int handPos, int *bonus) {
     ...
     j = state->hand[currentPlayer][choice1];  //store card we will trash
        ...
        discardCard(handPos, currentPlayer, state, 0); // discard from previous player

        //discard trashed card
        for (i = 0; i < state->handCount[currentPlayer]; i++) // off by 1 error introducedD
        {
...
int ambassadorEffect(int currentPlayer, int card, int choice1, int choice2, struct gameState *state, int handPos, int *bonus)
{
    ...
    for (i = 0; i < state->numPlayers; i++)           // extended loop to go beyond actual list of players
    {
        if (i != currentPlayer)
        {
    ...
    discardCard(handPos-1, currentPlayer, state, 0); // changed which position gets discarded
...
int tributeEffect(int currentPlayer, int nextPlayer, int *tributeRevealedCards, int card, int choice1, int choice2, struct gameState *state, int handPos, int *bonus)
{
    ...
    for (i = 0; i <= 2; i++)
    {
        ...
        { //Action Card
            state->numActions = state->numActions + 4; // added extra actions (was +2)
        }
    }
    return 0;
}
...
int minionEffect(int currentPlayer, int card, int choice1, int choice2, struct gameState *state, int handPos, int *bonus) {
        ...
        else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
        {
            ...
            for (i = 0; i < state->numPlayers; i++)
            {
                if (i != currentPlayer)
                {
                    if ( state->handCount[i] > 3 ) // lowered threshold for re-drawing qualification
                    {
                        //discard hand
                        ...
                        //draw 4
                        for (j = 0; j < 5; j++) // increased amount of cards that are drawn
                        {
                            drawCard(i, state);
                        }
                    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mineEffect(currentPlayer, card, choice1,choice2, state, handPos, bonus);
    ...
    case baron:
       ...
       return baronEffect(currentPlayer, choice1, choice2, state, handPos, bonus);
    ...
    case minion:
        return minionEffect(currentPlayer, card, choice1, choice2, state, handPos, bonus);
    ...
    case tribute:
        return tributeEffect(currentPlayer, nextPlayer, tributeRevealedCards, card, choice1, choice2, state, handPos, bonus);
    
    case ambassador:

        return ambassadorEffect(currentPlayer, card, choice1, choice2, state, handPos, bonus);
